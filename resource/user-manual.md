...
sure
to visit the link first).
It generates a txt file that contain the cards to be batch imported to Anki.

> Why not making it as an Anki add-on? Currently, there is no plan for that, as the utility consists of many parts.
> For example, it uses [QWebKitView](https://github.com/liuzikai/pyqt-macos-native-webview), wrapping macOS native
> WKWebView in PyQt. Building it as an Anki add-on may not bring much benefit but can introduce a lot of difficulty to
> test and debug.
...
* Choose a word on the left side to edit its card. All automations built in
  the [Maple Anki Template](https://github.com/liuzikai/Maple-Anki-Template) works after importing to Anki.
  For example, `cn.` in the Paraphase section will be highlighted as a part of speech (POS).
...
* Close the program at any time. Confirmed cards are saved to a txt file in the save directory
  ...
  Confirmed or Discarded words will no appear when you load the same data source next time. The other words remain.
* Import the generate txt file into Anki, following [these instructions](#import-to-anki).
...
* Anki User Directory: select your user directory (named with your Anki username). This is necessary to put generate
  audio files into the correct place.
* Save Directory: path to output the generated cards.
* Default Directory when Opening CSV: as title suggests. Save you some time if you are working on a CSV vocabulary list.
...
Maple Utility use macOS built-in text-to-speech to generate pronunciation audio.
...
Maple Utility support multiple data sources. The idea is to process the list of vocabulary.
...
The other words are remained unchanged.

## Kindle Vocabulary Builder

Connect Kindle to the Mac. Import the list using the `Kindle` button.

Kindle Vocabulary Builder records the example sentence and the citation, which will be import into the citation
textbox (at the bottom of Example).

## CSV

A CSV (comma-separated values) file can be used as a data source.
The CSV should have at least 5 columns: subject, usage, title, authors, category.
The additional columns are ignored.
The first line of the file is regarded as heading and gets discarded.
...
Therefore, I create a list in Things to stores those words captured from everywhere.
...
As name suggests, create a single entry manually by clicking `New` (`Cmd + N`).
...
word, but will lose when the program restarted (confirmed entries are written to the output file though, of course).
...
word. A button will appear on the right of the `Subject` textbox if the suggestion is available.
...
Place for explanations. There are a lot of automations in this area by
...
To add an image to the card (at the end of Paraphrase field), copy the image to the clipboard, and click on the area on
...
Place for usage of the word. There are a lot of automations in this area by
the [Maple Anki Template](https://github.com/liuzikai/Maple-Anki-Template).
...
This area support [Smart Duplication](#smart-duplication): create a new manual card based on the selected text.

## Hint

It's shown as a hint for your self in Spelling and Dictation. For example, two words have the similar explaination but
one starts with 'a' another starts with 'b'. Then you may want to put hints 'a_' and 'b_' for yourself.
...
Confirmed or Discarded words will no appear when you load the same data source next time.
...
Web query is one of the powerful feature of Maple Utility.
...
> Disclaimer: this tool only display the websites being queried. The contents may be copyrighted. Use the results at
> your own risks.
...
If such indicator is available, it's shown on the right of the `Subject` textbox.
...
To copy an image on the website, right click on the image, select `Copy Image`.
...
* The card type must be `Maple Template X`. If no such card type, you should import the sample deck of the [Maple Anki Template](https://github.com/liuzikai/Maple-Anki-Template) first.
